...
import { estimateGas, sendTransaction } from '@/wallet/chain/evm';
import { sendCosmosTransaction } from '@/wallet/chain/cosmos';
...
function SendPageContent() {
  ...
  const [txChainType, setTxChainType] = useState<'EVM' | 'Cosmos'>('EVM');
  ...
  // Uses same validation as receive page
  ...
  // Convert between EVM and Cosmos addresses using Injective SDK
  // Uses same implementation as receive page
  const convertAddress = () => {
    if (!recipient) return;
    
    try {
      if (isEvmAddress(recipient)) {
        // EVM to Cosmos using SDK (same as receive page)
        const cosmosAddress = getInjectiveAddress(recipient);
        setRecipient(cosmosAddress);
      } else if (isCosmosAddress(recipient)) {
        // Cosmos to EVM using SDK (same as receive page)
        const evmAddress = getEthereumAddress(recipient);
        setRecipient(evmAddress);
      } else {
        setError('Invalid address format. Please use EVM (0x...) or Cosmos (inj1...) address.');
      }
    } catch (err) {
      setError('Failed to convert address. Please check the address format.');
      console.error('Address conversion error:', err);
    }
  };

  // Convert cosmos address to EVM for gas estimation using SDK
  // Uses same implementation as receive page
  const getEvmAddress = useCallback((address: string): string => {
    ...
    if (isCosmosAddress(address)) {
      try {
        // Use SDK to convert Cosmos to EVM (same as receive page)
        return getEthereumAddress(address);
      } catch (err) {
        console.error('Failed to convert Cosmos address to EVM:', err);
  ...
  const handleEstimate = useCallback(async (useDefaults = false) => {
    ...
    if (!estimateRecipient || !estimateAmount || !address) {
      ...
      // Clear gas estimate when fields are missing
      setGasEstimate(null);
      setEstimating(false);
      return;
    }

    // Check if recipient is Cosmos address - skip EVM gas estimation
    if (isCosmosAddress(estimateRecipient)) {
      console.log('[Send] Cosmos address detected, skipping EVM gas estimation');
      // For Cosmos transactions, we'll use a default estimate or skip it
      // Cosmos transactions have fixed fees, typically around 0.0001 INJ
      setGasEstimate({
        gasLimit: BigInt(200000), // Default Cosmos gas limit
        maxFeePerGas: BigInt(500000000), // 0.5 Gwei equivalent
        maxPriorityFeePerGas: BigInt(0),
        totalCost: BigInt(100000000000000), // ~0.0001 INJ
      });
      setEstimating(false);
      return;
    }

    // Convert cosmos address to EVM for estimation (if needed)
    ...
    // Double-check: if conversion failed and address is still Cosmos format, skip EVM gas estimation
    if (isCosmosAddress(estimateRecipient)) {
      console.log('[Send] Address conversion failed, still Cosmos format, skipping EVM gas estimation');
      setGasEstimate({
        gasLimit: BigInt(200000),
        maxFeePerGas: BigInt(500000000),
        maxPriorityFeePerGas: BigInt(0),
        totalCost: BigInt(100000000000000),
      });
      setEstimating(false);
      return;
    }

    // Final validation: ensure address is valid EVM format before calling estimateGas
    if (!isEvmAddress(estimateRecipient)) {
      console.log('[Send] Invalid address format for EVM gas estimation:', estimateRecipient);
      // Set a default gas estimate even for invalid addresses to allow sending
      // The actual validation will happen in handleSend
      setGasEstimate({
        gasLimit: BigInt(21000), // Default EVM gas limit
        maxFeePerGas: BigInt(500000000), // 0.5 Gwei
        maxPriorityFeePerGas: BigInt(0),
        totalCost: BigInt(10500000000000), // ~0.0000105 INJ
      });
      setEstimating(false);
      // Don't set error here, let handleSend validate the address
      return;
    }
  ...
  const handleSendClick = () => {
    console.log('[Send] handleSendClick called:', {
      recipient,
      amount,
    ...
    if (loading || estimating) {
      console.log('[Send] Already processing, ignoring click');
      return;
    }
    
    // Validate inputs
    if (!recipient || !recipient.trim()) {
      console.log('[Send] Missing recipient');
      setError('Please fill in recipient address');
      return;
    }
    
    if (!amount || !amount.trim()) {
      console.log('[Send] Missing amount');
    ...
    if (!privateKey) {
      console.log('[Send] No private key available');
      setError('Wallet not unlocked');
      return;
    }
    
    // Validate address format
    if (!isEvmAddress(recipient) && !isCosmosAddress(recipient)) {
      // Try to see if it can be converted
      const converted = getEvmAddress(recipient);
      if (!isEvmAddress(converted) && !isCosmosAddress(recipient)) {
        console.log('[Send] Invalid address format:', recipient);
        setError('Invalid address format. Please use EVM (0x...) or Cosmos (inj1...) address.');
        return;
      }
    }
    ...
    if (isPinLocked || autoLockMinutes === 0) {
      console.log('[Send] Authentication required, showing modal');
      // Need authentication
      setShowAuthModal(true);
    } else {
      console.log('[Send] Within PIN-free window, sending directly');
  ...
  const handleSend = async () => {
    ...
    if (!recipient || !amount || !privateKey) {
      console.log('[Send] Missing required fields:', {
    ...
    try {
      let hash: string;
      let chainType: 'EVM' | 'Cosmos' = 'EVM';
      
      // Check address type and route to appropriate sending method
      // First check if it's a Cosmos address
      if (isCosmosAddress(recipient)) {
        console.log('[Send] Sending Cosmos transaction to:', recipient);
        chainType = 'Cosmos';
        ...
        // It's a valid EVM address
        console.log('[Send] Sending EVM transaction to:', recipient);
        chainType = 'EVM';
        ...
        // Try to convert - check if it's a Cosmos address that failed validation
        // or try to convert to EVM
        let convertedAddress: string | null = null;
        
        // First, try to see if it's a Cosmos address that can be converted
        if (recipient.startsWith('inj1')) {
          try {
            convertedAddress = getEthereumAddress(recipient);
            if (isEvmAddress(convertedAddress)) {
              console.log('[Send] Converted Cosmos to EVM and sending EVM transaction:', convertedAddress);
              chainType = 'EVM';
              hash = await sendTransaction(
                privateKey,
                convertedAddress,
              ...
              // Try Cosmos transaction with original address
              console.log('[Send] Attempting Cosmos transaction with original address:', recipient);
              chainType = 'Cosmos';
              hash = await sendCosmosTransaction(
                privateKey,
                recipient,
            ...
            console.error('[Send] Address conversion failed:', convErr);
            throw new Error('Invalid address format. Please use EVM (0x...) or Cosmos (inj1...) address.');
          }
        } else {
          // Not a Cosmos address, try to convert to EVM
          convertedAddress = getEvmAddress(recipient);
          if (isEvmAddress(convertedAddress)) {
            console.log('[Send] Converted and sending EVM transaction to:', convertedAddress);
            chainType = 'EVM';
            hash = await sendTransaction(
              privateKey,
              convertedAddress,
      ...
      console.log('[Send] Transaction successful:', { hash, chainType });
      setTxChainType(chainType);
      setTxHash(hash);
    } catch (err) {
          ...
          <div className="p-5 rounded-2xl bg-black border border-white/10 mb-6 space-y-3">
            ...
            <div className="flex justify-between items-center py-2 border-t border-white/5">
              ...
              <span className="text-sm font-bold text-white">
                {txChainType === 'Cosmos' ? 'Injective Cosmos' : 'Injective EVM'}
              </span>
            </div>
            <div className="flex justify-between items-center py-2 border-t border-white/5">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="space-y-6">
          ...
          <button
            ...
            disabled={!recipient || !amount || loading || estimating}
