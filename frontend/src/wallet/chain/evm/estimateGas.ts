...
export async function estimateGas(
  ...
  // Validate address format - EVM addresses must be 0x followed by 40 hex characters
  const evmAddressRegex = /^0x[a-fA-F0-9]{40}$/;
  if (!evmAddressRegex.test(to)) {
    throw new Error(
      `Invalid EVM address format: "${to}". Address must be a hex value of 20 bytes (40 hex characters) starting with 0x. ` +
      `This function only supports EVM addresses. For Cosmos addresses (inj1...), use Cosmos transaction functions instead.`
    );
  }
  if (!evmAddressRegex.test(from)) {
    throw new Error(
      `Invalid EVM address format for 'from': "${from}". Address must be a hex value of 20 bytes (40 hex characters) starting with 0x.`
    );
  }
